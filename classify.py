...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Adaptation Intelligence Platform - classify documents into taxonomy",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
TWO-STEP WORKFLOW (search first, classify later):

  Step 1 - download only (no LLM, no Azure):
    python classify.py --source google_cse_corporate \
  ...
  Step 2 - classify a single staged file:
    python classify.py --source corporate_pdf_direct \
...
""",
    ...
    if args.download_only:
        ...
        print("  python classify.py --source corporate_pdf_direct --path tmp/staged/<file>.txt")
        print("  python classify.py --source corporate_pdf_direct --path tmp/staged/ --all-staged")
